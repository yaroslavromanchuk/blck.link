...
$amount = abs($model->invoice->currency_id == 1 ? round($model->amount, 2) * $model->invoice->exchange : round($model->amount, 2));
...
<div class="header-page">
    ...
    <table width="100%">
        <tr width="100%">
            ...
            <td width="50%" style="text-align: right"><?= DateFormat::datumUah($model->invoice->date_pay) ?></td>
        </tr>
    </table>
    <br>
    ...
    <?php if ($model->artist->artist_type_id) {
    ...
    <p>уклали цей Акт-Звіт №<?=$model->artist->id . '/' . $model->invoice_id?> про нарахувння Роялті за період з <?= DateFormat::datumUah($quarterDate['start'])?> по <?= DateFormat::datumUah($quarterDate['end']) ?></p>
</div>
<div class="body-page">
...
$s2 = !empty($s[1]) ? $s[1] : '';
$s3 = !empty($s[2]) ? $s[2] : '';
//$s2 = mb_convert_encoding($s2, 'UTF-8', "UTF-8");
//$s2 = !empty($s2) ? substr($s2, 0, 1) : '';
//$s3 = !empty($s3) ? mb_strtoupper(substr($s3, 0, 1))  : '';

?>

<div class="footer">
    <p>Всього сума Роялті Правовласника за період з <?= DateFormat::datumUah($quarterDate['start'])?> по <?= DateFormat::datumUah($quarterDate['end']) ?> склала <?=$amount?> грн. <span style="">(<?=Number::num2str($amount)?>)</span>, без ПДВ, згідно ст.196 п.196.1.6. ПКУ, <?php if ($model->invoice->currency_id != 2) { echo 'що є еквівалентом ' . abs(round($model->amount, 2)) . ' '. $model->invoice->currency->currency_symbol . ' за курсом ' . $model->invoice->exchange . ' на дату підписання Акту про розподіл винагороди.'; } ?></p>
    <p>Сума, яка підлягає виплаті Правовласнику складає <b><?=$amount?> грн. <span>(<?=Number::num2str($amount)?>)</span></b>, без ПДВ, згідно ст.196 п.196.1.6. ПКУ.</p>
