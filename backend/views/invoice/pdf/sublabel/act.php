...
$amount = abs($model->currency_id != 2 ? round($model->total, 2) * $model->exchange : round($model->total, 2));
    ...
    <table class="table table-striped table-bordered" style="font-size: 8px">
        ...
        foreach ($tracks as $item) {
            ...
            <tr>
                ...
                <td><?=$item['prav2']?></td>
                <!--<td><?=$item['platform']?></td>
                <td><?php if ($model->currency_id == 2) {
                        echo DateFormat::getQuarterText($item['date_report']);
                    } else {
                        echo DateFormat::datumUah2($item['date_report']);
                    }
                ?>
                </td>-->
            </tr>
